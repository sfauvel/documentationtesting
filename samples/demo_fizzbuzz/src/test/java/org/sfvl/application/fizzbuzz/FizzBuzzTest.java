...
public class FizzBuzzTest extends ApprovalsBase {
    ...
    @Test
    public void output_in_a_table() {

        final int lastValue = 20;
        Function<IntFunction<String>, String> toLine = f -> IntStream.rangeClosed(1, lastValue)
                .mapToObj(f)
                .collect(Collectors.joining(" | ", "| ", ""));

        write("[%autowidth,cols=" + lastValue + "*^]",
                "|====",
                toLine.apply(Integer::toString),
                toLine.apply(FizzBuzz::get),
                "|====");

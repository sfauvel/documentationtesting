...
A script `check.sh` verify all differences to the head git version.
...
* Generate doc (no framework really needed, Xunit is just a tool to help us to run all generation)
* Check there is no difference
* Generate Html from adoc
